    ...
    <div className="w-72 flex-shrink-0 flex flex-col border-r border-border bg-[#0d1117]">
      ...
      <div className="p-4 flex flex-col gap-4">
        ...
        {offlineAlerts.length > 0 && (
          <div className="mt-4 border border-red-500 p-3 rounded-[3px]">
            ...
            {offlineAlerts.map((alert, i) => (
              <div
                key={i}
                className="text-[10px] text-white mb-2 border-b border-border pb-2"
              >
                <div>{alert.location}</div>
                <div>{alert.incident_type}</div>
                <div>{alert.casualties} casualties</div>
              </div>
            ))}
          </div>
        )}
